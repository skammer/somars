...
async fn fetch_channels() -> Result<Vec<Channel>> {
    ...
    println!("{}", json_str); // Debug print

    let channels_response: ChannelResponse = serde_json::from_str(&json_str)?;
    let channels: Vec<_> = channels_response.channels.into_values().collect();
    Ok(channels)
}
